...
export function registerWebhookHandlers(app: App): void {
  ...
  const onPR = ({ octokit, payload }: { octokit: any; payload: any }) => {
    ...
    void queue.add(() =>
      runPipeline(octokit, owner, repo, pr.number as number, pr.title as string, (pr.body as string | null) ?? '').catch(
        (err: unknown) => console.error(`[queue] Pipeline error ${owner}/${repo}#${pr.number as number}:`, err),
      ),
    );
  };
  ...
  app.webhooks.on('issue_comment.created', ({ octokit, payload }: { octokit: any; payload: any }) => {
    ...
    void queue.add(async () => {
      try {
        ...
        await runPipeline(octokit, owner, repo, prNum, pr.title as string, (pr.body as string | null) ?? '');
      } catch (err: unknown) {
